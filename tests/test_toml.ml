  ...
  [
    "Rache Methodology Approved" >::: [
      ...
      "String" >:: (fun () ->
         assert_equal
           (TString "\b")
           (get_value (Toml.parse "key=\"\\b\"") "key");
         assert_equal
           (TString "\t")
           (get_value (Toml.parse "key=\"\\t\"") "key");
         assert_equal
           (TString "\n")
           (get_value (Toml.parse "key=\"\\n\"") "key");
         assert_equal
           (TString "\r")
           (get_value (Toml.parse "key=\"\\r\"") "key");
         assert_equal
           (TString "\"")
           (get_value (Toml.parse "key=\"\\\"\"") "key");
         assert_equal

 ...
 * (see it uncommenting 'open Pprint' in test_toml.ml)
 *)

let rec string_of_table tbl =
  Hashtbl.fold (fun k v acc -> (k, v) :: acc) tbl []
  |> string_of_list (fun (k, v) -> k ^ "->" ^ string_of_entrie v)

and string_of_entrie = function
  | TTable tbl -> "TTable(" ^ string_of_table tbl ^ ")"
  | TValue v -> "TValue(" ^ string_of_val v ^ ")"

and string_of_list (stringifier : 'a -> string) (els : 'a list)  =
